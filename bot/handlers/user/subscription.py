...
async def my_subscription_command_handler(
    ...
    # Create keyboard with Mini App button for config
    config_link = active.get("config_link")
    keyboard_rows = []
    
    # Debug logging
    logging.info(f"Config link: {config_link}, Mini App URL: {settings.SUBSCRIPTION_MINI_APP_URL}")
    
    if config_link and settings.SUBSCRIPTION_MINI_APP_URL:
        # Extract subscription ID and pass it to Mini App
        subscription_id = config_link.split('/')[-1] if config_link else ""
        mini_app_url = f"{settings.SUBSCRIPTION_MINI_APP_URL}?subscription={subscription_id}"
        
        logging.info(f"Creating Mini App button with URL: {mini_app_url}")
        
        config_button = InlineKeyboardButton(
            text=get_text("open_subscription_config_button"),
            web_app=WebAppInfo(url=mini_app_url)
        )
        keyboard_rows.append([config_button])
    else:
        logging.warning(f"Mini App button not created. Config link: {config_link}, Mini App URL: {settings.SUBSCRIPTION_MINI_APP_URL}")
    
    # Add back button
    back_markup = get_back_to_main_menu_markup(current_lang, i18n)
    keyboard_rows.extend(back_markup.inline_keyboard)
